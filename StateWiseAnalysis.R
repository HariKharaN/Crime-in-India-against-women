...
statecrime <- function(indiacrime, row, state,crime) {
  ...
  plot(year,thecrime ,pch= 15, col="red", xlab = "Year", ylab= crime, main = atitle,xlim=c(2009,2020),ylim=c(ymin,ymax), axes=FALSE)
  
  # Set the axes
  axis(side=1, at=c(2008:2020))
  ...
  # Projected crime incidents from 2021 to 2025 using a linear regression model
  ...
  legend( x="topleft", legend=c(alegend),col=c("red"), bty="n" , lwd=1, lty=c(2),pch=c(15) )
  ...
  print("The projected Crime rate in Upcoming Years(2021 to 2025) :")
  print(nthecrime)
